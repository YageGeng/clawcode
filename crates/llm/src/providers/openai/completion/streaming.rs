    ...
    ) -> Result<streaming::StreamingCompletionResponse<StreamingCompletionResponse>, CompletionError>
    {
        ...
        if enabled!(Level::TRACE) {
            tracing::trace!(
                ...
                serde_json::to_string_pretty(&request_as_json).context(SerializeSnafu{stage:"trace-openai"})?
            );
        }

        let req_body = serde_json::to_vec(&request_as_json).context(SerializeSnafu {
            stage: "serialize-openai",
        })?;
        ...
        let req = self
            ...
            .body(req_body)
            .context(HttpSnafu {
                stage: "openai-completion-http",
            })
            .context(ClientSnafu {
                stage: "openai-completion-body",
            })?;
