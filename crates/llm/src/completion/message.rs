...
impl Image {
    pub fn try_into_url(self) -> Result<String, MessageError> {
        match self.data {
            ...
            DocumentSourceKind::Base64(data) => {
                let Some(media_type) = self.media_type else {
                    return Err(MessageError::ConversionError {
                        msg: "A media type is required to create a valid base64-encoded image URL"
                            .to_string(),
                    });
                };
            ...
            unknown => Err(MessageError::ConversionError {
                msg: format!("Tried to convert unknown type to a URL: {unknown:?}"),
            }),
        }
    }
}
...
pub enum MessageError {
    #[snafu(display("Message conversion error: {msg}"))]
