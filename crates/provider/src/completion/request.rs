...
//!
//! Most applications use [`Prompt`], [`Chat`], or [`TypedPrompt`] directly on a
//! [`CompletionModel`]. Provider integrations implement
...
pub trait Completion<M: CompletionModel> {
    ...
    ///
    /// For example, a request builder created by a higher-level wrapper may
    /// already contain a system prompt or tool configuration.
    fn completion<I, T>(
        &self,
        prompt: impl Into<Message> + WasmCompatSend,
        chat_history: I,
