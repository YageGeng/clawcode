//! Provider-owned request and response message models.
//!
//! These types moved out of the agent protocol so provider wire compatibility
//! remains independent from runtime transcript messages.
...
/// Converts one provider-neutral message into one or more provider-specific messages.
pub trait TryIntoMany<T> {
    /// Error returned when conversion fails.
    type Error;

    /// Consumes the source message and converts it into provider-specific messages.
    fn try_into_many(self) -> Result<Vec<T>, Self::Error>;
}

/// Converts one provider-neutral message into provider-specific messages infallibly.
...
impl From<MessageError> for CompletionError {
    /// Preserves message conversion failures as completion request errors.
