...
async fn in_memory_store_exposes_incremental_messages_before_turn_finalizes() {
    ...
    let interim_messages = store
        .load_messages(session_id.clone(), thread_id.clone(), 10)
        .await
        .unwrap();
    assert_eq!(
        interim_messages,
    ...
    store
        .finalize_turn(session_id.clone(), thread_id.clone(), Usage::new())
        .await
        .unwrap();
...
async fn in_memory_store_take_pending_input_keeps_non_pending_continuations_queued() {
    ...
    let error = store
        .take_pending_input(session_id.clone(), thread_id.clone())
        .await
    ...
    let continuation = store
        .take_continuation(session_id, thread_id)
        .await
        .unwrap();
    assert_eq!(
        continuation,
