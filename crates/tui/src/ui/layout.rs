...
pub(super) struct FrameRows {
    ...
    /// Area used for the inline picker below the composer.
...
pub(super) fn frame_rows(area: Rect, composer_text: &str) -> Option<FrameRows> {
    frame_rows_with_picker(area, composer_text, 0)
}

/// Splits the terminal frame and optionally reserves rows for an inline picker.
pub(super) fn frame_rows_with_picker(
    area: Rect,
    composer_text: &str,
    picker_height: u16,
) -> Option<FrameRows> {
    let picker_height = picker_height.min(5);
    ...
    match rows.as_ref() {
        [transcript, top_status, composer, picker, bottom_status] => Some(
            FrameRows::builder()
...
mod tests {
    ...
    /// Verifies visible picker reserves rows under the composer.
    #[test]
    fn frame_rows_reserves_picker_height_when_visible() {
        let rows = frame_rows_with_picker(Rect::new(0, 0, 80, 20), "hello", 3)
            .expect("rows");

        assert_eq!(rows.picker.height, 3);
        assert!(rows.picker.y > rows.composer.y);
    }
