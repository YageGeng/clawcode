...
| `config` | 类型化配置，从 `~/.config/clawcode/config.toml` 或 `./claw.toml` 加载，基于 Figment，通过 `ArcSwap` 共享 |
...
创建 `~/.config/clawcode/config.toml`。如果需要仓库内临时配置，可以创建 fallback 文件 `./claw.toml`：
